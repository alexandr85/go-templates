...
func TestRoundSignificant(t *testing.T) {
	tests := []struct {
		...
		{0.000123456, -2, 0},
		{0.000123456, -1, 0},
		{0.000123456, 0, 0.0001},
		{0.000123456, 1, 0.00012},
		{0.001234567, 5, 0.00123457},
	...
	for _, tt := range tests {
		rounded := RoundSignificant(tt.value, tt.precision)
		assert.InDelta(t, tt.expected, rounded, 1e-10, "value: %v, precision: %v", tt.value, tt.precision)
	}
}
